...
class Device {
   public:
    ...
    ~Device() {
        if (active_port) {
            delete active_port;
            kassert(ibv_close_device(ctxt) == 0);
        }
    }
       ...
       public:
        class Chunk {
           public:
            ...
            uint32_t zero_read(char *buf, uint32_t len);
        ...
        class Cluster {
           public:
            ...
            bool is_my_buffer(const char *c) const { return c >= base && c < end; }

            bool is_valid_chunk(const Chunk *c) const { return c >= chunk_base && c < chunk_base + num_chunk; }
        ...
        class MemPoolContext {
           ...
           public:
            ...
            explicit MemPoolContext(MemoryManager *m) : perf_logger(nullptr), manager(m), n_bufs_allocated(0) {}
