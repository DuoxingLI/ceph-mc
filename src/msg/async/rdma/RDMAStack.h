...
class RDMADispatcher {
   ...
   public:
    ...
    explicit RDMADispatcher(CephContext *cct, std::shared_ptr<Infiniband> &ib);
    ...
    void make_pending_worker(RDMAWorker *w) {
        std::lock_guard<std::mutex> l{w_lock};
        auto it = std::find(pending_workers.begin(), pending_workers.end(), w);
        if (it != pending_workers.end()) return;
...
class RDMAWorker : public Worker {
    ...
    class C_handle_cq_tx : public EventCallback {
       ...
       public:
        ...
        void do_request(uint64_t fd) { worker->handle_pending_message(); }
    };
   ...
   public:
    ...
    void notify_worker() { center.dispatch_event_external(tx_handler); }
};

struct RDMACMInfo {
    RDMACMInfo(rdma_cm_id *cid, rdma_event_channel *cm_channel_, uint32_t qp_num_) : cm_id(cid), cm_channel(cm_channel_), qp_num(qp_num_) {}
    rdma_cm_id *cm_id;
...
class RDMAConnectedSocketImpl : public ConnectedSocketImpl {
   ...
   public:
    RDMAConnectedSocketImpl(CephContext *cct, std::shared_ptr<Infiniband> &ib, std::shared_ptr<RDMADispatcher> &rdma_dispatcher, RDMAWorker *w);
...
class RDMAServerSocketImpl : public ServerSocketImpl {
   ...
   public:
    RDMAServerSocketImpl(CephContext *cct, std::shared_ptr<Infiniband> &ib, std::shared_ptr<RDMADispatcher> &rdma_dispatcher, RDMAWorker *w,
                         entity_addr_t &a);
...
class RDMAStack : public NetworkStack {
    ...
    virtual Worker *create_worker(CephContext *cct, unsigned worker_id) override;
