...
struct Context;
namespace r_common::PerfCounter {
...
enum class unit_t : uint8_t { UNIT_BYTES, UNIT_NONE };
...
class PerfCounters {
   public:
    ...
    void set_prio_adjust(int p) { prio_adjust = p; }
...
class SortPerfCountersByName {
   public:
    bool operator()(const PerfCounters *lhs, const PerfCounters *rhs) const { return (lhs->get_name() < rhs->get_name()); }
};
...
class PerfGuard {
    const r_common::real_clock::time_point start;
    PerfCounters *const counters;
    const int event;

   public:
    PerfGuard(PerfCounters *const counters, const int event) : start(r_common::real_clock::now()), counters(counters), event(event) {}

    ~PerfGuard() { counters->tinc(event, r_common::real_clock::now() - start); }
};
